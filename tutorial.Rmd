...
trainNetwork(train_type = "lm", # train a language model
             ...
             validation.split = 0.2, # use 20% train size for validation
             run.name = "lm_16S_target_right",
             batch.size = 512, 
             epochs = 8, 
             steps.per.epoch = 25, # 1 epoch = 20 batches
...
trainNetwork(train_type = "lm", # train a language model
             ...
             batch.size = 512, 
             epochs = 8, 
             steps.per.epoch = 25, # 1 epoch = 25 batches
             step = 100, # take a sample every 100 steps
             output = list(none = FALSE,
                           checkpoints = TRUE,
                           tensorboard = TRUE,
...
```{r warning = FALSE, message = FALSE}
model_path <- "Ep.005-val_loss0.00-val_acc0.999.hdf5"
...
In the following example we use the binary model trained to classify 16S/bacteria to predict the location of 16S sequences (similar to https://github.com/tseemann/barrnap). We iterate over a new bacteria file (not present in train or validation data)
and make predictions every 100 steps
...
We can now plot the model confidence in 16S over the whole genome. 
...
Let's visualize or models predictions
...
cp <- list.files(file.path(checkpoint_path, "16S_vs_bacteria_checkpoints"),
                 full.names = TRUE)
model_path <- cp[length(cp)]
model <- keras::load_model_hdf5(model_path, compile = FALSE)
model <- keras::load_model_weights_hdf5(model, model_path)

fasta_path <- file.path(path_16S_validation, "GCF_000427035.1_09mas018883_genomic.16s.fasta.fasta")
...
for (i in 1:(seq_len - 500)) {
  ...
  df <- data.frame(position = (1:(nrow(attribution_mask))) + i - 1, ig_sum = attribution_mask[ , 1])
  ig_list[[i]] <- df
}
